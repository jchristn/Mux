...
> **Beta software**
> mux is in beta. APIs, interfaces, configuration formats, tool schemas, and CLI behavior are all subject to change. Feedback is welcome via [issues](https://github.com/jchristn/Mux/issues) and [discussions](https://github.com/jchristn/Mux/discussions).
...
- Universal prompt management (`v0.12.3`): every string mux feeds a model — the persona, the compaction framing, the tool descriptions, the little tool-result payloads — is legible and editable, never buried in code. A single catalog defines every prompt with a sensible default (grouped by kind), overrides persist to `~/.mux/prompts.json`, and each surface (terminal, desktop, web, VS Code) can browse the catalog and edit or reset any entry; prompt profiles now expose all three persona fields. Editable over REST via `/v1.0/api/prompts/catalog`. See `docs/PROMPTS.md`
- Smart large-file context (`v0.12.3`): a file too big to inline becomes a navigable **map** (a structural outline with line ranges, so the model reads exact ranges with `read_file(offset, limit)`) instead of being truncated or refused — with **summarize** (a cached map-reduce summary that keeps line pointers) and **truncate** modes selectable per a `context` settings group. `read_file` returns a map by default, and `POST /v1.0/api/context/file` lets thin clients (the VS Code extension) offload mapping/summarizing to the server
