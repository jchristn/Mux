...
- **Phases 2–7 — done.** Chat panel (streaming + in-editor approvals + session binding), context providers (now including an LSP `symbols` source), the seven inline commands + code actions, the session tree (resume/rename/duplicate/export/delete), and the endpoint picker. Diff review opens the SCM view, and **turn-level undo/redo now works through the server**: each run records a git checkpoint in the workspace, and `mux.undo`/`mux.redo` restore a turn's file changes via `GET/POST /v1.0/api/checkpoints[/undo|/redo]`, backed by a server-lifetime `CheckpointRegistry` keyed by working directory. Terminal context is reported unavailable by design.
- **Phase 8 — done.** Locale registry, locale-aware formatters (tested), and the twelve baseline locales translated for the command palette, settings, and runtime UI (manifest `package.nls.*` + runtime `l10n/bundle.l10n.*`), plus a pseudo-locale and a CI key-drift check. Command prompts sent to the model stay in English by design; the CJK/Arabic/Devanagari strings are a first pass that a native review can refine.
- **Phase 9 — done.** Sixteen unit tests over the vscode-free logic pass under `node --test`; the `@vscode/test-electron` harness and an activation/command suite compile and run where VS Code can be downloaded (CI uses `xvfb-run`).
- **Phase 10 — CI authored, publish pending tokens.** `.github/workflows/vscode-extension.yml` builds, checks, packages, and on a `vscode-v*` tag publishes to the Marketplace and Open VSX with a checksummed release. `vsce package` is verified locally. Publishing needs the two tokens (see below).
- **Phase 11 — done.** `README.md`, `CHANGELOG.md`, `LICENSE.md`, `docs/VSCODE.md`, the mux `README`/`IMPROVEMENTS.md` mentions, and the usemux.ai site (the editor as a fifth surface) are all in.
- **Phase 12 — one done, two deferred with cause.** LSP-aware context shipped. The other two are blocked on capabilities that do not exist, and building half of either would mislead more than help:
  - *Live session mirroring* would stream a run started in the TUI or desktop into the editor. Those surfaces run `Mux.Core` in-process and never touch `mux serve`, so there is no shared event bus for the WebSocket to bridge. Mirroring cross-surface runs needs a cross-process event bus first; over SSE the server already streams the runs it drives, which is what the panel uses. Deferred until that bus exists.
  - *Propose-before-write* (stage a turn's edits for per-hunk accept/reject before they touch disk) needs the agent loop to emit edits as proposals rather than applying them — a `Mux.Core` dry-run write mode that does not exist. Turn-level undo (above) covers the after-the-fact case today. Deferred until the loop supports staged edits.
