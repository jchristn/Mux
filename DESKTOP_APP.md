...
>
> **Remaining:** `ToolRuntimeBinder` (step 3, folds `McpRuntime`/`SkillRuntime`/`ExternalToolsBinder` + the `Program.cs ApplyTemplate` coordinator), the **`ConversationController`** (step 4) with its in-place TUI turn-loop adoption, and the **endpoint→`AgentLoopOptions` resolver** (`CommandRuntimeResolver` ⇄ the desktop's `AgentLoopTurnRunner`, which does a simplified copy — promoting it fixes the desktop's missing prompt-profile/tool-description parity but needs a settings-decoupling DTO). Each of these rewires shipping turn/resolution paths, so they land with an interactive smoke-test rather than blind.
