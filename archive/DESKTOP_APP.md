...
> **Status: DELIVERED — plan archived (2026-09-12).** The desktop app has reached the plan's goals: full chat with streaming/thinking/tools/approvals, threads, tasks, every configuration manager, MCP tools + skills in turns, usage analytics + stats, undo/redo, command palette, prompt history, local server, comprehensive 11-language internationalization, a high-contrast accessibility theme, and **true parallel tabs**. User-facing guidance lives in [`docs/DESKTOP.md`](../docs/DESKTOP.md).
>
> **Remaining items are human-gated, not code:** (1) code-signing / notarization / per-OS installers / auto-update — needs the maintainer's certificates (unsigned self-contained builds via `publish-desktop.*` already work); (2) a native-speaker review pass over the machine-produced non-English translations; (3) live-GUI smoke-testing of the concurrent parallel-tab streaming and the screen-reader/keyboard-nav accessibility pass — neither can be verified headless. This document is retained for history; it is no longer an active worklist.
...
**Increment 2b — true parallel tabs (DELIVERED 2026-09-12):** each open thread now owns a `TabContext` (`src/Mux.Desktop/Shell/TabContext.cs`) holding its own `AgentLoopTurnRunner` + `ConversationService` + transcript panel + streaming widgets + turn timers + title/persistence state, so tabs run agent turns **concurrently** and switching never blocks. The per-conversation fields in `MainWindow` are proxied to `Ctx` (the render target: the tab whose event is being applied via `_RenderCtx`, else `_Active`), so the existing render/turn code operates on the right tab without a rewrite. Background-tab events route through `OnTabEvent`/`_RenderCtx` into their own transcript; visible-only controls (scroll, empty state, context indicator) are gated on `RenderingActive`. `SendAsync` scopes the render context to synchronous UI bursts (`WithContext`), never held across awaits, so concurrent turns don't clobber each other; only the same tab blocks a second turn. Because all tabs share one git working tree, checkpoint recording is serialized (`_CheckpointGate`); undo/redo remain global. Closing a busy tab cancels its turn; a layout rebuild drops contexts and re-opens the active thread. `ConversationServiceSuite` gained a per-tab isolation case. **Its concurrent-streaming behavior cannot be verified headless — needs a live GUI smoke test.**
...
>
> **Step 4 engine (2026-09-12):** the **`Mux.Core.Conversation.ConversationController`** is built and tested — the shared, genuinely-async turn/queue orchestrator (history, pending queue, stats, checkpoint + hook gating around a front-end `TurnOutcome` executor). It centralizes the turn-history policy (record a completed turn even when its answer is empty; drop only cancelled/errored turns) that both front ends now implement inline; a 9-case suite covers it. **Remaining:** the in-place TUI/desktop adoption of the controller (rewiring `EnqueueOrRun`/`RunTurn`/`OnTurnComplete` and the desktop `ConversationService` to delegate to it) — that rewires shipping interactive control flow, so it lands with interactive smoke-testing rather than blind.
...
**Phase 11 — i18n rollout & accessibility.** All catalogs, RTL/CJK/expansion QA, keyboard nav, focus order, contrast, responsive widths. *Exit:* i18n Definition of Done met; a11y pass green. _Delivered (i18n):_ the desktop now ships the same eleven languages as the `mux serve` dashboard (en, es, pt, fr, it, de, zh, ar, ru, ms, hi). The dashboard's authored chrome packs (134 keys × 11 langs) are ported verbatim into `DashboardStrings` and merged with hand-authored desktop-specific strings (`DesktopStrings`: splash, sidebar, composer, About, chat labels) in `LocalizationService`; long help text stays English by design, matching the dashboard's translated/untranslated boundary. A header language selector switches locale live (rebuilding the shell), flips Arabic to RTL, and persists to `desktop.json` (`localeCode`). Japanese (`ja`) was removed to match the dashboard's exact language set.

Coverage was then deepened to the **whole app**: an ambient `Localizer.T(key)` (like `AppTheme.Current`) lets every window/dialog localize without ctor plumbing. Every manager window, form, and dialog is localized — Endpoints, MCP, Skills, Subagents, Prompts, Pricing, Search, Plugins, Hooks, Commands, Keybindings, the Usage dashboard and Stats, the Settings and About windows, and the shared dialogs — plus MainWindow's labels, tooltips, and notices. Dropdowns keep canonical persisted values while localizing only the display; format placeholders (`{0}`) are preserved across languages. The authored catalog (`tools/i18n/desktop-i18n.js` → generated `DesktopStrings.cs`) is ~660 keys, all translated in all 11 languages. Verified by `DesktopLocalizationSuite`, including a `FullCatalogParity` case that asserts every catalog key resolves (non-blank, non-key) in every locale. _Accessibility (delivered):_ a **high-contrast theme** variant (`AppTheme.CreateHighContrast`: black surfaces, white text/borders, bright-yellow accent) selectable from Settings › Theme and persisted; icon-only buttons (header glyphs, sidebar nav) expose their localized tooltip/label as an `AutomationProperties.Name` for screen readers. The playful `WelcomeQuips` stay English by design. **Remaining (needs live-GUI QA):** keyboard-nav / focus-order verification with a screen reader.
