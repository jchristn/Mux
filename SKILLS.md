...
- **Phase 6 — default library — done.** `DefaultSkillLibrary` seeds the full curated, cross-platform catalog on first run through `EnsureConfigDirectory`, preserving user edits on re-seed. The library is now the complete 46-skill catalog from Section 10.1, authored as data through `DefaultSkillBuilder`/`DefaultSkillCommandDef` and split across focused per-category files — `DefaultGitSkills` (15), `DefaultDotnetSkills` (8), `DefaultHygieneSkills` (8), `DefaultScaffoldDocsSkills` (7), and `DefaultWorkflowUtilitySkills` (8) — which `DefaultSkillLibrary.All()` merges (rejecting any duplicate id). Skills carry multiple commands where the catalog calls for them (for example `git-status-vs-head` → `summarize`/`files-changed`, `git-stash-manager` → `save`/`list`/`pop`/`show`). Git and .NET skills use `pwsh` for cross-platform reach and guard the default branch on destructive operations; JSON/text utilities use `node`. Covered by `DefaultSkillsSuite` (all validate, one runs, re-seed preserves edits — the validate case counts `DefaultSkillLibrary.All().Count` dynamically), and every default is validated in CI by `mux skill validate`. Seeding runs on **every** startup, not only first creation, so defaults shipped in a later release reach an existing `~/.mux`: `DefaultSkillLibrary.SeedNewInto` records seeded ids in a `.seeded-defaults` manifest inside the skills directory, so newly shipped defaults arrive on upgrade while a default the user has deleted is not resurrected and existing edits are never overwritten (covered by upgrade and delete-survival cases).
...
All seven phases are implemented, tested, and committed on `feature/skills`, including the complete 46-skill default library and in-app editing.
...
**Security.** `SkillSecuritySuite` treats a skill as untrusted input even though it is user-authored. It proves the trust boundary at two points: the resolver appends every caller argument as its own `ProcessStartInfo.ArgumentList` entry with `UseShellExecute` off, and end-to-end through `SkillExecutor` a set of shell metacharacters (`;`, `$(…)`, backticks, `&&`) arrives at the interpreter verbatim and unexpanded — a shell would have split, chained, or substituted them. Path traversal and the interpreter allowlist are held by `SkillLoaderSuite`'s validation matrix (id/script-escape and bad-interpreter cases). The `SkillsDirectory` override is resolved through a single `SettingsLoader.ResolveSkillsDirectory` shared by the host and the CLI, with `SkillRuntimeSuite` asserting the override wins and null/whitespace falls back to the default.
