...
- [ ] **0.1** Create working branch `feature/run-lifecycle`. — Owner: ___ — Notes: ___
- [ ] **0.2** Bump product version `0.11.0` → `0.12.0` in `src/Directory.Build.props` (`<Version>`) and
  `src/Mux.Core/Settings/Defaults.cs` (`ProductVersion`). — Owner: ___ — Notes: ___
- [ ] **0.3** Bump `src/Mux.VSCode/package.json` `version` `0.11.3` → `0.12.0`. — Owner: ___ — Notes: ___
- [ ] **0.4** Add an `## Unreleased` → `## 0.12.0` heading scaffold in `CHANGELOG.md` (fill as phases land).
  — Owner: ___ — Notes: ___
- [ ] **0.5** Confirm baseline: `dotnet build` clean (0 warnings) both TFMs; full Touchstone suite green;
...
- [ ] **1.1** Add `Mux.Core.Agent.AgentEventSerializer` (one public static class, one file) holding the
...
- [ ] **1.2** Change `Mux.Cli.Commands.StructuredOutputFormatter` to **delegate** to the Core serializer for
...
- [ ] **1.3** Expose a typed entry point the server can call without the CLI dependency, e.g.
...
- [ ] **1.4** Tests — `AgentEventSerializerSuite` in `src/Test.Shared/Suites/` (register in `MuxSuites.All`):
...
- [ ] **2.1** Add `Mux.Server.Runs.RunStatusEnum` (`Running`, `AwaitingApproval`, `Completed`, `Failed`,
  `Canceled`) — one enum, one file. — Owner: ___ — Notes: ___
- [ ] **2.2** Add `Mux.Server.Runs.RunHandle` — `RunId`, `SessionId`, `EndpointName`, `Model`, `Status`
  ...
  — Owner: ___ — Notes: ___
- [ ] **2.3** Add `Mux.Server.Runs.RunRegistry` — `ConcurrentDictionary<string, RunHandle>` with
...
- [ ] **2.4** Refactor `ChatRoutes.StreamChatAsync`: create a `RunHandle` (linked CTS from `ctx.Token`),
...
- [ ] **2.5** Emit a new SSE `run` event **first** carrying `{ runId, sessionId }` so the browser/editor can
  address the run for cancel. (Additive; existing clients ignore unknown events.) — Owner: ___ — Notes: ___
- [ ] **2.6** Register the `RunRegistry` singleton in `MuxServer` wiring and pass it into `ChatRoutes`.
  — Owner: ___ — Notes: ___
- [ ] **2.7** Tests — `RunRegistrySuite`:
...
- [ ] **3.1** Add `Mux.Server.Routes.RunRoutes` (one class, one file) and register in `MuxServer`.
  — Owner: ___ — Notes: ___
- [ ] **3.2** `POST /v1.0/api/runs/{runId}/cancel` → `ApiAuth.Authorize` → `registry.TryCancel(runId)`:
...
- [ ] **3.3** Confirm the MCP/tool executor receives the handle token so a long-running tool actually
  interrupts (the executor signature already takes a `CancellationToken`). — Owner: ___ — Notes: ___
- [ ] **3.4** Tests (extend `MuxServerRouteSuite`): cancel a live scripted run → 200 + terminal state within
...
- [ ] **4.1** Add DTOs `Mux.Server.Models.RunStateReply` and `RunListReply` (one class per file), projecting
...
- [ ] **4.2** `GET /v1.0/api/runs` → list active + recently-terminal runs (summary rows). — Owner: ___ —
  Notes: ___
- [ ] **4.3** `GET /v1.0/api/runs/{runId}` → full `RunStateReply`; 404 on unknown. Both authed. — Owner: ___
  — Notes: ___
- [ ] **4.4** Tests: list reflects an active run; detail shows correct status through a scripted
...
- [ ] **5.1** Replace the connect/ack stub `HandleWebSocketAsync` in `MuxServer.cs` with a real bridge.
...
- [ ] **5.2** **Auth the upgrade** — `/v1.0/ws` currently has no API-key check. Require the key via query
  param (`?apiKey=`) or subprotocol header; reject the upgrade otherwise. Keep loopback binding. — Owner:
  ___ — Notes: ___
- [ ] **5.3** Subscription protocol: inbound `{ "action": "subscribe", "runId": "..." }` (also accept
...
- [ ] **5.4** Approvals over WS: emit `approval_required`, accept inbound
...
- [ ] **5.5** Backpressure: bounded per-subscriber channel; a slow/dead subscriber is dropped, never stalls
  the run. Document the policy in XML. — Owner: ___ — Notes: ___
- [ ] **5.6** Tests: a WS subscriber to a scripted run receives the full ordered event set and the **payloads
...
- [ ] **6.1** In `DashboardPage.cs`: capture the `runId` from the new SSE `run` event; keep the Stop button
...
- [ ] **6.2** Show live run status (Running / Awaiting approval / Canceled) near the composer, driven by the
  stream. — Owner: ___ — Notes: ___
- [ ] **6.3** Localize any new UI strings through the existing dashboard i18n path (11 languages) per
  `I18N.md`; run the i18n check. — Owner: ___ — Notes: ___
- [ ] **6.4** Manual/automated: verify Stop actually stops server-side (run no longer in `GET /runs`), not
  just the browser. — Owner: ___ — Notes: ___

### Phase 7 — VS Code extension

- [ ] **7.1** `ApiClient` (`src/Mux.VSCode/src/api/ApiClient.ts`): add `cancelRun(runId)` →
  `POST /v1.0/api/runs/{runId}/cancel`; capture `runId` from the stream's `run` event. — Owner: ___ —
  Notes: ___
- [ ] **7.2** Add a `mux.cancelRun` command + Stop affordance in the chat webview
...
- [ ] **7.3** Opt-in **live session mirroring** over the WebSocket bridge: subscribe by `sessionId`, render
  ...
  Notes: ___
- [ ] **7.4** Tests: `node --test` unit case for `cancelRun` (positive: posts to the right URL with the key;
...
- [ ] **8.1** TUI/CLI: confirm headless JSONL output is unchanged after the Phase 1 delegation — the golden
  parity test (1.4) is the gate; add a `print --output-format jsonl` regression case if not already covered.
  — Owner: ___ — Notes: ___
- [ ] **8.2** Desktop: verify in-process Stop still cancels a turn (no regression from shared types). — Owner:
  ___ — Notes: ___
- [ ] **8.3** Desktop (optional): adopt `RunStatusEnum` vocabulary for the per-tab attention model and add a
...
- [ ] **9.1** `src/Mux.Server/Documentation/ApiDoc.cs`: add tag + per-route metadata for `POST /runs/{id}/cancel`,
...
- [ ] **9.2** `docs/REST_API.md`: add a **Runs** section (three routes: method, path, params, request/response
  ...
  — Notes: ___
- [ ] **9.3** `assets/postman/mux.postman_collection.json`: add a **Runs** folder (cancel/list/detail) with
...
- [ ] **9.4** `docs/VSCODE.md`: document the Stop/cancel command and the `mux.liveMirror` setting. — Owner:
  ___ — Notes: ___
- [ ] **9.5** `README.md` + `CHANGELOG.md` (`0.12.0`): describe cancel-run, run-state inspection, and the
  WebSocket event bridge. — Owner: ___ — Notes: ___
- [ ] **9.6** Update `IMPROVEMENTS.md`: mark the WebSocket bridge, cancel route, and task-state rows in the
...
- [ ] **10.1** `dotnet build` clean, 0 warnings, both TFMs (net8.0 + net10.0). — Owner: ___ — Notes: ___
- [ ] **10.2** Full Touchstone suite green (new suites registered in `MuxSuites.All`); record
  **final count: ___** and the delta vs baseline (0.5). — Owner: ___ — Notes: ___
- [ ] **10.3** VS Code: `npm run compile && npm test` (unit) green; `npm run i18n` clean; `npm run lint`
  clean. — Owner: ___ — Notes: ___
- [ ] **10.4** Dashboard i18n check clean. — Owner: ___ — Notes: ___
- [ ] **10.5** OpenAPI sanity: start `mux serve`, fetch `/openapi.json`, confirm the three new routes are
  present and the document validates. — Owner: ___ — Notes: ___

### Phase 11 — Final: automated verification, hand-off, publish

- [ ] **11.1** Run every automated check I can (Phase 10) and paste the results into the Notes here. — Owner:
  ___ — Notes: ___
- [ ] **11.2** Produce the **manual test checklist** (§5) and notify Joel of exactly what needs human
...
- [ ] **11.3** Package the VS Code extension (`npm run package` → `.vsix`) and **notify Joel to publish** it
